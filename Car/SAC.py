...
def find_losses(id: int, policy_nets: list[nn.Module], value_nets: list[nn.Module], target_value_nets: list[nn.Module],
    ...
    action_probs = [policy_nets[j](state.get_state_tensor(device)) for j in range(game_manager.CarState.NUM_AGENTS)]

    y = torch.zeros(game_manager.CarState.NUM_AGENTS, device=device)
...
def train(device: str):
    # Temperatures
    log_alphas = [torch.tensor(np.log(ALPHA_INITIAL), dtype=torch.float, requires_grad=True)] * game_manager.CarState.NUM_AGENTS
    ...
    value_losses_over_time = [[] for _ in range(game_manager.CarState.NUM_AGENTS)]

    # Used to average losses when checking for convergence
    sum_policy_losses = [0] * game_manager.CarState.NUM_AGENTS
    sum_value_losses = [0] * game_manager.CarState.NUM_AGENTS
